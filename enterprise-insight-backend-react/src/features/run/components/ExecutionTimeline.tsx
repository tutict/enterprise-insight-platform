...
export default function ExecutionTimeline({ steps }: { steps: TimelineStep[] }) {
  ...
  if (!steps.length) {
    return (
      <section className="panel p-5">
        ...
        <div className="mt-4 rounded-lg border border-slate-700/80 bg-console-950 p-4 text-sm text-slate-500">
          {t('timeline.empty')}
        </div>
      </section>
    )
  ...
  return (
    <section className="panel p-5">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <button
              key={step.key}
              type="button"
              className={`relative min-h-[92px] min-w-0 rounded-lg border px-3 py-3 text-left transition hover:bg-slate-800/50 ${
            ...
            >
              {nextStep ? (
                <span
                  className={`pointer-events-none absolute left-[calc(50%+1rem)] top-7 hidden h-0.5 w-[calc(100%-2rem)] xl:block ${connectorClass[step.status]}`}
                />
              ) : null}
      ...
      <div className="mt-4 rounded-lg border border-slate-700/80 bg-console-950 p-3">

...
function DslEditor() {
  ...
  const compileLoading = useRunStore((state) => state.compileLoading)
      ...
      <section className="panel p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            type="button"
            onClick={() => void compileCurrentDsl()}
        ...
        {compileError ? (
          <div className="mt-4 rounded-lg border border-red-400/30 bg-red-950/60 p-3 text-sm text-red-100">
            ...
            <p className="mt-1 text-red-100/80">{compileError}</p>
          </div>
        ) : null}
      </section>
        ...
        <div className="panel p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded-md border border-white/10 px-2 py-1 text-xs text-slate-400">
              {prompt ? 'compiled' : 'idle'}
            </span>
          </div>
          <CodeBlock value={prompt} emptyLabel="Compile a DSL to inspect the returned prompt." collapsible />
        </div>

        <div className="panel p-5">
          <h3 className="mb-3 text-sm font-semibold text-slate-100">Compiler Response</h3>
          <CodeBlock
            value={compileResult ? JSON.stringify(compileResult.dsl, null, 2) : ''}
